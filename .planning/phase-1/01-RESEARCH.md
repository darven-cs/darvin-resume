...
# 项目初始化（自动生成前端模板）
wails init -n Darvin-Resume -t vue-ts
...
// main.go — 注册绑定
func main() {
    store := infrastructure.NewSQLiteStore("Darvin-Resume.db")
...
import (
    ...
    "Darvin-Resume/internal/bridge"
    "Darvin-Resume/internal/domain"
    "Darvin-Resume/internal/infrastructure"
)
...
func main() {
    ...
    dbPath := filepath.Join(configDir, "Darvin-Resume", "Darvin-Resume.db")
...
2. **数据库文件存储位置**
   ...
   - Recommendation: Phase 1 先使用默认路径（`~/.config/Darvin-Resume/` on Linux, `%APPDATA%/Darvin-Resume/` on Windows），后续 Phase 可增加自定义路径
